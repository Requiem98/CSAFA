...
class CircConv2d(nn.Module):
    def __init__(self, in_channels:int, out_channels:int, kernel_size:tuple, padding:tuple = (1,1,0,0)): 
        super().__init__()
...
class N_cir_conv(nn.Module):
    ...
    def __init__(self,in_channels,out_channels,N = 2, padding:tuple = (1,1), pool:bool = True):
        ...
        model = []
        model.append(CircConv2d(in_channels,out_channels,kernel_size=(3,3)))
        model.append(nn.ReLU(True))
        for i in range(N-1):
            model.append(CircConv2d(out_channels,out_channels,kernel_size=(3,3)))
            model.append(nn.ReLU(True))
...
class VGG16_cir_MSAFA_PCA(nn.Module):
    def __init__(self, img_size:tuple, dimension : int, out_dim:int, *args, **kargs):
        super().__init__()
        
        self.cnn = VGG16_cir(in_channels=3, init_weights=True)
        self.sa = MaxSpatialAware((img_size[0] // 32) * (img_size[1] // 32), dimension) #2*8 if downsize = 2, 4*16 if downsize = 1
        self.pca = LearnablePCA(512*dimension, out_dim) 
