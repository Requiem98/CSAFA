...
class Semi_Siamese_3ViT(nn.Module):
    def __init__(self, img_size:tuple, out_dim:int = 1000, *args, **kwargs):
        super().__init__()
        
        self.vit_q = vit_base_patch16(img_size, drop_rate = 0.1, attn_drop_rate = 0.1, num_classes = out_dim)   #query
        self.vit_p = vit_base_patch16(img_size, drop_rate = 0.1, attn_drop_rate = 0.1, num_classes = out_dim//2)   #polar
        self.vit_gen = vit_base_patch16(img_size, drop_rate = 0.1, attn_drop_rate = 0.1, num_classes = out_dim//2) #generated pano
            
    def forward(self, q, p, gen):
        q_embedd = self.vit_q(q)
        p_embedd = self.vit_p(p)
        gen_embedd = self.vit_gen(gen)

        gt_embedd = torch.hstack([p_embedd, gen_embedd])
        
        return f.normalize(q_embedd, dim=1), f.normalize(gt_embedd, dim=1)
        



class Siamese_3ViT_v0_l(pl.LightningModule):
    def __init__(self, img_size:dict, out_dim:int = 1000, *args, **kargs):
        ...
        self.example_input_array = (torch.Tensor(1, 3, img_H, img_W), torch.Tensor(1, 3, img_H, img_W), torch.Tensor(1, 3, img_H, img_W))
        
        self.model = Semi_Siamese_3ViT(img_size = (img_H, img_W), out_dim = out_dim, *args, **kargs)
    ...
    def training_step(self, batch, batch_idx):
        ...
        ###FIRST STEP####
        latent_variable, gt_latent_variable = self.model(query, p, gen)
